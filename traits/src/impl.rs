 ...
 * BufRead<File> or [u8], but it's not important what type R is, and R is only used to declare the
 * type of src, so the function can also be written as:
 *
 * */
...
/*
 * As a return type
 * If your function return a type that implements MyTrait. you can write its return type as -> impl
 * MyTrait. This can help simplify your type signature quite a lot!
 * */
...
Before impl Trait syntax, you had to allocate on the heap in order to return a closure. But now you can do it all statically,
like this:
* */
